...
fn main() {
    ...
    let result_a = handle_a.join().expect("error A");
    let result_b = handle_b.join().expect("error B");

    // //but also:
    // let result_a = handle_a.join.unwrap();
    // let _ignored_result = handle_a.join();
    ...
    let (trans, recv) = channel::unbounded();
    ...
    handle_1.join().expect("error1");
    handle_2.join().expect("error2");
